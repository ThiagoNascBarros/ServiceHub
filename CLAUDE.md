...
dotnet build FlencyHub.sln

# Run the API (Development mode, opens Swagger at http://localhost:5062/swagger)
dotnet run --project src/FlencyHub.Api/FlencyHub.Api.csproj

# Run with HTTPS profile
dotnet run --project src/FlencyHub.Api/FlencyHub.Api.csproj --launch-profile https

# Restore packages
dotnet restore FlencyHub.sln

# Clean build artifacts
dotnet clean FlencyHub.sln
```

No test projects exist yet. When added, run them with:
```bash
dotnet test FlencyHub.sln
# Single test by name
...
| Project | Role |
|---|---|
| `FlencyHub.Domain` | Entities, value objects, domain events, repository interfaces, domain services |
| `FlencyHub.Application` | Use cases / application services, DTOs, command/query handlers (CQRS if adopted) |
| `FlencyHub.Infrastructure` | EF Core DbContext, repository implementations, external service clients |
| `FlencyHub.Api` | ASP.NET Core controllers, middleware, DI wiring, Swagger/OpenAPI |
...
- Controllers live in `src/FlencyHub.Api/Controllers/` and use `[ApiController]` + `[Route("[controller]")]`.
